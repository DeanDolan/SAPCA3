...
function registerFailure(user, ip){
  ...
  r.count += 1
  if(r.count>=MAX_FAILS){ r.lockUntil = now()+LOCK_MS; metrics.lockedOut += 1; logWarn('lockout',{ user, ip }) }
  attempts.set(kk, r)
}
...
app.post('/api/secure/login', secureLoginLimiter, async (req,res)=>{
  ...
  try{
    ...
    if(!rows.length){ metrics.loginFailed++; registerFailure(username,ip); logInfo('auth_fail',{type:'secure',reason:'user_not_found',ip}); return res.json({ok:false,message:GENERIC}) }
    const ok = await bcrypt.compare(String(password||''), rows[0].password_hash)
    if(!ok){ metrics.loginFailed++; registerFailure(username,ip); logInfo('auth_fail',{type:'secure',reason:'bad_password',ip}); return res.json({ok:false,message:GENERIC}) }
